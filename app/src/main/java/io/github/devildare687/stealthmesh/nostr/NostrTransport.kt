...
class NostrTransport(
    ...
    companion object {
        ...
        private const val READ_ACK_INTERVAL = io.github.devildare687.stealthmesh.util.AppConstants.Nostr.READ_ACK_INTERVAL_MS // ~3 per second (0.35s interval like iOS)
        ...
        transportScope.launch {
            try {
                ...
                val recipientPeerIDForEmbed = try {
                    io.github.devildare687.stealthmesh.favorites.FavoritesPersistenceService.shared
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = try {
                io.github.devildare687.stealthmesh.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }
            if (selected !is io.github.devildare687.stealthmesh.geohash.ChannelID.Location) {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            io.github.devildare687.stealthmesh.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            if (ContactIdentityResolver.isNoiseKeyHex(peerID)) {
                val noiseKey = ContactIdentityResolver.bytesFromHex(peerID) ?: return null
                val favoriteStatus = io.github.devildare687.stealthmesh.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (ContactIdentityResolver.isMeshPeerId(peerID)) {
                val fallbackStatus = io.github.devildare687.stealthmesh.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
