...
class MessageManager(private val state: ChatState) {
    ...
    private val MESSAGE_DEDUP_TIMEOUT = io.github.devildare687.stealthmesh.util.AppConstants.UI.MESSAGE_DEDUP_TIMEOUT_MS // 30 seconds
    private val SYSTEM_EVENT_DEDUP_TIMEOUT = io.github.devildare687.stealthmesh.util.AppConstants.UI.SYSTEM_EVENT_DEDUP_TIMEOUT_MS // 5 seconds
    ...
    fun addMessage(message: BitchatMessage) {
        ...
        try { io.github.devildare687.stealthmesh.services.AppStateStore.addPublicMessage(message) } catch (_: Exception) { }
    }
    ...
    fun addChannelMessage(channel: String, message: BitchatMessage) {
        ...
        try { io.github.devildare687.stealthmesh.services.AppStateStore.addChannelMessage(channel, message) } catch (_: Exception) { }
        ...
        val viewingGeohashChannel = try {
            if (channel.startsWith("geo:")) {
                ...
                selected is io.github.devildare687.stealthmesh.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
            } else false
    ...
    fun addPrivateMessage(peerID: String, message: BitchatMessage) {
        ...
        val accepted = try {
            io.github.devildare687.stealthmesh.services.AppStateStore.addPrivateMessage(
                conversationID,
                message
            )
        } catch (_: Exception) {
    ...
    suspend fun addPrivateMessageDurably(
        ...
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        val accepted = try {
            io.github.devildare687.stealthmesh.services.AppStateStore.addPrivateMessageDurably(
    ...
    fun clearPrivateMessages(peerID: String) {
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        io.github.devildare687.stealthmesh.services.AppStateStore.deletePrivateConversation(conversationID)
    ...
    fun updateMessageDeliveryStatus(messageID: String, status: DeliveryStatus) {
        ...
        if (updated) {
            ...
            try { io.github.devildare687.stealthmesh.services.AppStateStore.updatePrivateMessageStatus(messageID, status) } catch (_: Exception) { }
        }
    ...
    fun removeMessageById(messageID: String) {
        ...
        run {
            ...
            if (changed) {
                ...
                io.github.devildare687.stealthmesh.services.AppStateStore.removePrivateMessage(messageID)
            }
        }
        // Channels
        run {
