...
import io.github.devildare687.stealthmesh.model.PrivateMessagePacket
...
class MeshCore(
    ...
    private fun setupDelegates() {
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                try { io.github.devildare687.stealthmesh.services.AppStateStore.setTransportPeers(transport.id, peerIDs) } catch (_: Exception) { }
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerInfoFromVerifiedAnnouncement(
                ...
                isVerified: Boolean,
                capabilities: io.github.devildare687.stealthmesh.model.PeerCapabilities?
            ): Boolean {
            ...
            override fun onDeliveryAckReceived(messageID: String, peerID: String) {
                try {
                    io.github.devildare687.stealthmesh.services.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        io.github.devildare687.stealthmesh.model.DeliveryStatus.Delivered(peerID, java.util.Date())
                    )
                } catch (_: Exception) { }
            ...
            override fun onReadReceiptReceived(messageID: String, peerID: String) {
                try {
                    io.github.devildare687.stealthmesh.services.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        io.github.devildare687.stealthmesh.model.DeliveryStatus.Read(peerID, java.util.Date())
                    )
                } catch (_: Exception) { }
    ...
    fun sendVerifyChallenge(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val payload = NoisePayload(
            ...
            data = io.github.devildare687.stealthmesh.services.VerificationService.buildVerifyChallenge(noiseKeyHex, nonceA)
        )
        sendNoisePayloadToPeer(payload, peerID)
    }

    fun sendVerifyResponse(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val tlv = io.github.devildare687.stealthmesh.services.VerificationService.buildVerifyResponse(noiseKeyHex, nonceA) ?: return
        val payload = NoisePayload(
    ...
    private fun buildAnnouncementPayload(announcement: IdentityAnnouncement, nickname: String): ByteArray? {
        ...
        try {
            if (directPeersForGossip.isNotEmpty()) {
                tlvPayload += io.github.devildare687.stealthmesh.services.meshgraph.GossipTLV.encodeNeighbors(directPeersForGossip)
            }
    ...
    private fun getDirectPeerIDsForGossip(): List<String> {
        return try {
            ...
            try { io.github.devildare687.stealthmesh.services.AppStateStore.setTransportDirectPeers(transport.id, localDirect) } catch (_: Exception) { }
            val union = try {
                io.github.devildare687.stealthmesh.services.AppStateStore.getDirectPeers().ifEmpty { localDirect }
