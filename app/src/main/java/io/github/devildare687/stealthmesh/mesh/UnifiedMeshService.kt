...
class UnifiedMeshService(
    ...
    override fun sendFavoriteNotification(peerID: String, isFavorite: Boolean) {
        val myNpub = try {
            io.github.devildare687.stealthmesh.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)?.npub
        } catch (_: Exception) {
            null
        }
    ...
    private fun mergedPeerIDs(): List<String> {
        ...
        try { ids.addAll(io.github.devildare687.stealthmesh.services.AppStateStore.peers.value) } catch (_: Exception) { }
