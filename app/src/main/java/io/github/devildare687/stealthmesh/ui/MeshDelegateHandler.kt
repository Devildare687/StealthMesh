...
import io.github.devildare687.stealthmesh.ui.NotificationTextUtils
...
class MeshDelegateHandler(
    ...
    private suspend fun processPeerUpdate(mergedPeers: List<String>) {
        ...
        runCatching { io.github.devildare687.stealthmesh.services.MessageRouter.tryGetInstance()?.onPeersUpdated(mergedPeers) }

        // Clean up channel members who disconnected
        channelManager.cleanupDisconnectedMembers(mergedPeers, getMyPeerID())

        runCatching { io.github.devildare687.stealthmesh.services.AppStateStore.canonicalizePrivateChats() }
        ...
        state.getSelectedPrivateChatPeerValue()?.let { currentPeer ->
            ...
            if (canonical != currentPeer) {
                io.github.devildare687.stealthmesh.services.ConversationAliasResolver.unifyChatsIntoPeer(
                    state = state,
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        io.github.devildare687.stealthmesh.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
        ...
        if (shouldSendReadReceipt) {
            ...
            try {
                val meshPeerID = ContactDirectory.resolve(senderConversationID).meshPeerID
                    ?: senderPeerID.takeIf {
                        io.github.devildare687.stealthmesh.services.ContactIdentityResolver.isMeshPeerId(it)
                    }
                if (meshPeerID != null &&
     ...
     */
    fun getPeerInfo(peerID: String): io.github.devildare687.stealthmesh.mesh.PeerInfo? {
