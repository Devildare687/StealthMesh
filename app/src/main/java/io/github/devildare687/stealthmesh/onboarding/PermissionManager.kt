...
class PermissionManager(private val context: Context) {
    ...
    private fun shouldRequireWifiAwarePermission(): Boolean {
        ...
        val enabled = try {
            io.github.devildare687.stealthmesh.ui.debug.DebugPreferenceManager.getWifiAwareEnabled(false)
        } catch (_: Exception) {
            false
        }
        ...
        return try {
            io.github.devildare687.stealthmesh.wifiaware.WifiAwareSupport.isSupported(context)
        } catch (_: Exception) {
            false
        }
    }
        ...
        if (shouldRequireWifiAwarePermission()) {
            ...
            categories.add(
                PermissionCategory(
                    ...
                    description = "Optional Wi‑Fi discovery for supported nearby devices.",
                    permissions = wifiAwarePermissions,
                    isGranted = wifiAwarePermissions.all { isPermissionGranted(it) },
                    systemDescription = "Allow StealthMesh to discover nearby Wi‑Fi devices"
                )
            )
        }
