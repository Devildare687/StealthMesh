package io.github.devildare687.stealthmesh.nostr
...
import io.github.devildare687.stealthmesh.services.SeenMessageStore
import io.github.devildare687.stealthmesh.ui.ChatState
import io.github.devildare687.stealthmesh.ui.PrivateChatManager
import io.github.devildare687.stealthmesh.ui.PrivateMessageOrigin
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: io.github.devildare687.stealthmesh.ui.DataManager,
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch {
            try {
                ...
                if (packet.type != io.github.devildare687.stealthmesh.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                io.github.devildare687.stealthmesh.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                if (file != null) {
                    ...
                    val savedPath = io.github.devildare687.stealthmesh.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BitchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
                    ...
                    if (!admitted) {
                        io.github.devildare687.stealthmesh.features.file.FileUtils.deleteStoredMediaPaths(
                            application,
                            listOf(savedPath)
                        )
                    }
