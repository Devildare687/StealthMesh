...
class BluetoothConnectionManager(
    ...
    private fun isGattServerEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { io.github.devildare687.stealthmesh.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true })
    }

    private fun isGattClientEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { io.github.devildare687.stealthmesh.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true })
    }
    ...
    init {
        ...
        try {
            val dbg = io.github.devildare687.stealthmesh.ui.debug.DebugSettingsManager.getInstance()
