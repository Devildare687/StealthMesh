package io.github.devildare687.stealthmesh.protocol
...
data class BitchatPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = BitchatPacket(
            ...
            route = route,
            ttl = io.github.devildare687.stealthmesh.util.AppConstants.SYNC_TTL_HOPS, // Use fixed TTL=0 for signing to ensure relay compatibility
...
object BinaryProtocol {
    ...
    fun encode(packet: BitchatPacket, padding: Boolean = true): ByteArray? {
        try {
            val maxPayloadLength = io.github.devildare687.stealthmesh.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH
    ...
    private fun decodeCore(
        ...
        try {
            ...
            if (payloadLength > io.github.devildare687.stealthmesh.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length ${payloadLength} exceeds maximum allowed (${io.github.devildare687.stealthmesh.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
            ...
            val payload = if (isCompressed) {
                ...
                val maxExpandedSize = io.github.devildare687.stealthmesh.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH
