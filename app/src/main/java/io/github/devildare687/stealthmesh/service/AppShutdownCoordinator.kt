...
object AppShutdownCoordinator {
    ...
    fun requestFullShutdownAndKill(
        ...
        val job = scope.launch {
            ...
            try {
                val intent = android.content.Intent(io.github.devildare687.stealthmesh.util.AppConstants.UI.ACTION_FORCE_FINISH)
                    .setPackage(app.packageName)
                app.sendBroadcast(intent, io.github.devildare687.stealthmesh.util.AppConstants.UI.PERMISSION_FORCE_FINISH)
            } catch (_: Exception) { }

            // Stop mesh (best-effort)
            try { mesh?.stopServices() } catch (_: Exception) { }
            try { io.github.devildare687.stealthmesh.nostr.NostrRelayManager.shared.disconnect() } catch (_: Exception) { }
            try { io.github.devildare687.stealthmesh.mesh.PowerManager.getInstance(app).shutdown() } catch (_: Exception) { }
            ...
            try { io.github.devildare687.stealthmesh.services.AppStateStore.clear() } catch (_: Exception) { }
