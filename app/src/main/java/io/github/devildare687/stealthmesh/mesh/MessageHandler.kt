...
import io.github.devildare687.stealthmesh.favorites.FavoriteControlMessage
...
import io.github.devildare687.stealthmesh.sync.PacketIdUtil
import io.github.devildare687.stealthmesh.util.toHexString
import io.github.devildare687.stealthmesh.features.voice.LiveVoiceManager
import io.github.devildare687.stealthmesh.features.voice.LiveVoiceScope
...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket): Boolean {
        ...
        try {
            ...
            val noisePayload = io.github.devildare687.stealthmesh.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                io.github.devildare687.stealthmesh.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = io.github.devildare687.stealthmesh.model.PrivateMessagePacket.decode(noisePayload.data)
                ...
                io.github.devildare687.stealthmesh.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = io.github.devildare687.stealthmesh.model.BitchatFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "Encrypted file from $peerID: ${file.fileSize} bytes")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = io.github.devildare687.stealthmesh.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = io.github.devildare687.stealthmesh.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                io.github.devildare687.stealthmesh.model.NoisePayloadType.VOICE_FRAME -> {
                ...
                io.github.devildare687.stealthmesh.model.NoisePayloadType.PEER_STATE -> {
                ...
                io.github.devildare687.stealthmesh.model.NoisePayloadType.DELIVERED -> {
                ...
                io.github.devildare687.stealthmesh.model.NoisePayloadType.READ_RECEIPT -> {
                ...
                io.github.devildare687.stealthmesh.model.NoisePayloadType.VERIFY_CHALLENGE -> {
                    delegate?.onVerifyChallengeReceived(peerID, noisePayload.data, packet.timestamp.toLong())
                }
                io.github.devildare687.stealthmesh.model.NoisePayloadType.VERIFY_RESPONSE -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = io.github.devildare687.stealthmesh.model.NoisePayload(
                type = io.github.devildare687.stealthmesh.model.NoisePayloadType.DELIVERED,
    ...
    suspend fun handleAnnounceWithResult(routed: RoutedPacket): AnnounceHandlingResult {
        ...
        try {
            val neighborsOrNull = io.github.devildare687.stealthmesh.services.meshgraph.GossipTLV.decodeNeighborsFromAnnouncementPayload(packet.payload)
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = BitchatPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = io.github.devildare687.stealthmesh.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = io.github.devildare687.stealthmesh.protocol.MessageType.fromValue(packet.type) == io.github.devildare687.stealthmesh.protocol.MessageType.FILE_TRANSFER
            val file = io.github.devildare687.stealthmesh.model.BitchatFilePacket.decode(packet.payload)
            if (file != null) {
    ...
    private suspend fun handlePrivateMessage(packet: BitchatPacket, peerID: String) {
        try {
            val isFileTransfer = io.github.devildare687.stealthmesh.protocol.MessageType.fromValue(packet.type) ==
                io.github.devildare687.stealthmesh.protocol.MessageType.FILE_TRANSFER
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                io.github.devildare687.stealthmesh.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, control.isFavorite)
                if (control.npub != null) {
                    io.github.devildare687.stealthmesh.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, control.npub)
                    io.github.devildare687.stealthmesh.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, control.npub)
                }

                val rel = io.github.devildare687.stealthmesh.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                ...
                val sys = io.github.devildare687.stealthmesh.model.BitchatMessage(
                    sender = "system",
                    content = notice,
                ...
                try {
                    val conversationID = io.github.devildare687.stealthmesh.services.ContactDirectory
                        .canonicalConversationId(fromPeerID)
                    val sysPrivate = io.github.devildare687.stealthmesh.model.BitchatMessage(
                        sender = "system",
                        content = notice,
                        timestamp = java.util.Date(),
                        isRelay = false,
...
interface MessageHandlerDelegate {
    ...
    fun updatePeerInfoFromVerifiedAnnouncement(
        ...
        isVerified: Boolean,
        capabilities: io.github.devildare687.stealthmesh.model.PeerCapabilities? = null
    ): Boolean
