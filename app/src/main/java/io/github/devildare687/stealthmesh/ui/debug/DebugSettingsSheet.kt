package io.github.devildare687.stealthmesh.ui.debug
...
import io.github.devildare687.stealthmesh.services.meshgraph.MeshGraphService
...
import io.github.devildare687.stealthmesh.util.DistributionInfoProvider
...
fun MeshTopologySection(
    ...
    val wifiAwareConnected by io.github.devildare687.stealthmesh.wifiaware.WifiAwareController.connectedPeers.collectAsState()
...
fun DebugSettingsSheet(
    ...
    val wifiAwareSupported by io.github.devildare687.stealthmesh.wifiaware.WifiAwareController.supported.collectAsState()
    val wifiAwareAvailable by io.github.devildare687.stealthmesh.wifiaware.WifiAwareController.available.collectAsState()
    val wifiAwareSupportStatus by io.github.devildare687.stealthmesh.wifiaware.WifiAwareController.supportStatus.collectAsState()
        ...
        if (isPresented) {
            ...
            while (true) {
                ...
                val devices = entries.map { (address, isClient, rssi) ->
                    ...
                    io.github.devildare687.stealthmesh.ui.debug.ConnectedDevice(
                ...
                try {
                    val ctrl = io.github.devildare687.stealthmesh.wifiaware.WifiAwareController
    ...
    BitchatBottomSheet(
        ...
        Box(modifier = Modifier.fillMaxWidth()) {
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        val running by io.github.devildare687.stealthmesh.wifiaware.WifiAwareController.running.collectAsState()
                        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                            ...
                            AssistChip(
                                onClick = { io.github.devildare687.stealthmesh.wifiaware.WifiAwareController.getService()?.sendBroadcastAnnounce() },
                                enabled = running,
                                label = { Text("Announce") }
                            )
                        }
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        val p = remember(gcsFpr) { io.github.devildare687.stealthmesh.sync.GCSFilter.deriveP(gcsFpr / 100.0) }
                        val nmax = remember(gcsFpr, gcsMaxBytes) { io.github.devildare687.stealthmesh.sync.GCSFilter.estimateMaxElementsForSize(gcsMaxBytes, p) }
