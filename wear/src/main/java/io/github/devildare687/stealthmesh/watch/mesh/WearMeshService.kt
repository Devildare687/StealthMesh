package io.github.devildare687.stealthmesh.watch.mesh
...
import io.github.devildare687.stealthmesh.mesh.BluetoothConnectionManager
import io.github.devildare687.stealthmesh.mesh.BluetoothConnectionManagerDelegate
...
class WearMeshService private constructor(private val context: Context) {
    ...
    var onPrivateMessage: ((io.github.devildare687.stealthmesh.model.BitchatMessage) -> Unit)? = null
    ...
    private fun handleMessageReceived(
        message: io.github.devildare687.stealthmesh.model.BitchatMessage
    ): Boolean = try {
        when {
            message.isPrivate -> {
     ...
     */
    fun sendFilePrivateEncrypted(recipientPeerID: String, file: io.github.devildare687.stealthmesh.model.BitchatFilePacket) {
        serviceScope.launch {
            ...
            val transferId = io.github.devildare687.stealthmesh.mesh.MeshPacketUtils.sha256Hex(
            ...
            while (System.currentTimeMillis() < prepDeadline) {
                when (val prep = meshCore.prepareFilePrivate(recipientPeerID, file, transferId, allowLegacyFallback = false)) {
                    is io.github.devildare687.stealthmesh.mesh.PrivateMediaPreparation.Ready -> {
                        prep.transfer.commit()
                        return@launch
                    }
                    io.github.devildare687.stealthmesh.mesh.PrivateMediaPreparation.AwaitingPeerState,
                    io.github.devildare687.stealthmesh.mesh.PrivateMediaPreparation.NeedsHandshake -> {
                        if (prep == io.github.devildare687.stealthmesh.mesh.PrivateMediaPreparation.NeedsHandshake) {
                            try { initiateNoiseHandshake(recipientPeerID) } catch (_: Exception) { }
                        }
                        delay(500)
                    }
